...
import it.tramways.analysis.roadmap.points.NetworkPoint;
...
public class RoadMapUtilities {
    ...
    private static List<NetworkPoint> findDestinations(List<NetworkPoint> points, LaneType type) {
        return points.stream()
                ...
                .filter(point -> hasChoiceProperty(AvailabilityAnalysisProperties.LANE_TYPE, type.name()).test(point.getSources().iterator().next()))
                .collect(Collectors.toList());
    }

    public static Predicate<Configurable> hasChoiceProperty(AvailabilityAnalysisProperties name, String value) {
    ...
    private static List<NetworkPoint> findSources(List<NetworkPoint> points, LaneType type) {
        return points.stream()
                ...
                .filter(point -> hasChoiceProperty(AvailabilityAnalysisProperties.LANE_TYPE, type.name()).test(point.getDestinations().iterator().next()))
                .collect(Collectors.toList());
    }

    public static NetworkPoint findNetworkPoint(tramways.core.model.roadmap.NetworkMap map, String id) {
